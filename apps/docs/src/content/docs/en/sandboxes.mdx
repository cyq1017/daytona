...
- **Networking**: TCP connection established from the parent sandbox to the child sandbox using the child sandbox's ID as the host. The traffic stays on the internal link network on the runner. For example: `telnet DEVICE_SANDBOX_ID 5555` reaches port `5555` on the linked child sandbox.
