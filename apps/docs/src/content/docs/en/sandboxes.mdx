...
Daytona provides methods to link sandboxes to existing sandboxes. A linked sandbox is attached to an existing parent sandbox at creation time.
...
Daytona provides Android sandboxes for running Android emulators alongside a standard Linux development environment.

- **Base sandbox**: a standard Linux sandbox where your code runs (git, builds, POSIX tooling)
- **Device sandbox(es)**: one or more Android emulator devices, attached to the base sandbox as [linked sandboxes](#linked-sandboxes)

You run your code in the base sandbox and reach each device over adb.
...
- **Snapshots**: Base: `android-base`. Devices (x86_64, AOSP, the numbers correspond to Android SDK levels / OS versions):

  `android-device-12-aosp`, `android-device-13-aosp`, `android-device-14-aosp`, `android-device-15-aosp`, `android-device-16-aosp`
- **adb from the base**: connect using the device sandbox ID and port `5555`: 

  `adb connect DEVICE_SANDBOX_ID:5555`
