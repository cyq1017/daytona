...
#### \_experimental\_pause()

```ts
_experimental_pause(timeout?: number): Promise<void>
```
...
#### \_experimental\_resume()

```ts
_experimental_resume(timeout?: number): Promise<void>
```
...
- If the resume operation fails or times out
