...
export class SandboxVolumeMount {
  ...
  // Encrypted stable identifier returned by the Archil control plane when
  // the mount token was minted. Required to revoke the token via
  // `DELETE /api/disks/{id}/users/token?identifier={identifier}`.
  @Column({ type: 'text', nullable: true })
