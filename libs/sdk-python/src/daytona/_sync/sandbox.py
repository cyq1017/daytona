...
class Sandbox(SandboxDto):
    ...
    @with_instrumentation()
    def _experimental_pause(self, timeout: float = 60) -> None:
    ...
    @with_instrumentation()
    def _experimental_resume(self, timeout: float = 60) -> None:
        ...
        start_time = time.time()
        _ = self._sandbox_api.resume_sandbox(self.id, _request_timeout=timeout if timeout > 0 else None)
        self.refresh_data()
