...
class AsyncSandbox(SandboxDto):
    ...
    @with_instrumentation()
    async def _experimental_pause(self, timeout: float = 60) -> None:
    ...
    @intercept_errors(message_prefix="Failed to resume sandbox")
    @with_instrumentation()
    async def _experimental_resume(self, timeout: float = 60) -> None:
        ...
        start_time = time.time()
        _ = await self._sandbox_api.resume_sandbox(self.id, _request_timeout=timeout if timeout > 0 else None)
        await self.refresh_data()
        ...
        check_interval = 0.1
        wait_start = time.time()
        while self.state == "resuming":
            await self.refresh_data()
            if self.state == "error":
                raise DaytonaError(
                    f"Sandbox {self.id} resume failed with state: {self.state}, error reason: {self.error_reason}"
                )
            if 0 < remaining <= time.time() - wait_start:
                raise DaytonaError(f"Sandbox {self.id} failed to resume within {timeout} seconds")
            await asyncio.sleep(check_interval)
            check_interval = min(check_interval * 1.5, 1.0)
